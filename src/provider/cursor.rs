//! Cursor provider - AI-first code editor
//!
//! Cursor is an AI-first code editor with CLI and headless mode for automation.
//!
//! Installation:
//! - Download from https://cursor.com
//! - CLI: `cursor` command available after installation
//!
//! Configuration:
//! - Cursor account authentication (via `cursor login`)
//! - `CURSOR_MODEL`: Model to use (optional)
...
/// Cursor AI code editor provider
...
impl CursorProvider {
    ...
    pub fn new() -> Self {
        ...
        Self {
            command: std::env::var("CURSOR_COMMAND").unwrap_or_else(|_| "cursor".to_string()),
    ...
    fn build_command(&self, prompt: &str, context: &Context) -> Command {
        ...
        // Add extra arguments
        if let Ok(extra_args) = std::env::var("CURSOR_EXTRA_ARGS") {
    ...
    async fn execute(&self, prompt: &str, context: &Context) -> Result<ProviderResponse> {
        ...
        let output = tokio::time::timeout(self.timeout, async {
            tokio::task::spawn_blocking(move || cmd.output())
                ...
                .map_err(|e| anyhow!("Failed to execute cursor: {}", e))
        })
        .await
        .map_err(|_| anyhow!("Cursor timed out after {:?}", self.timeout))??;
...
/// Parse modified files from Cursor output
fn parse_modified_files(output: &str) -> Vec<String> {
    ...
    for line in output.lines() {
        ...
        if let Some(path) = line.strip_prefix("Created: ") {
            ...
            files.push(path.trim().to_string());
        } else if let Some(path) = line.strip_prefix("Edited: ") {
            files.push(path.trim().to_string());
        }
    }
    files
...
mod tests {
    ...
    fn cleanup_env() {
        std::env::remove_var("CURSOR_COMMAND");
        std::env::remove_var("CURSOR_MODEL");
        std::env::remove_var("CURSOR_EXTRA_ARGS");
        std::env::remove_var("CURSOR_TIMEOUT_SECS");
    }
    ...
    fn test_cursor_provider_new() {
        ...
        assert_eq!(provider.command, "cursor");
    ...
    fn test_cursor_build_command() {
        ...
        assert!(args.contains(&std::ffi::OsStr::new("--headless")));
