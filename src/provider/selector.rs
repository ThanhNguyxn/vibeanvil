...
impl TaskType {
    ...
    pub fn required_capabilities(&self) -> Vec<Capability> {
        match self {
            TaskType::CodeGeneration => vec![Capability::CodeGeneration, Capability::Streaming],
            TaskType::CodeReview => vec![Capability::CodeReview, Capability::CodeExplanation],
            TaskType::Refactoring => vec![Capability::Refactoring, Capability::CodeGeneration],
            TaskType::BugFix => vec![
                Capability::BugFixing,
                Capability::Debugging,
                Capability::CodeGeneration,
            ],
            TaskType::TestWriting => vec![Capability::TestGeneration, Capability::CodeGeneration],
            TaskType::Documentation => vec![Capability::Documentation],
            TaskType::ArchitectureDesign => vec![Capability::Architecture, Capability::Planning],
            TaskType::Planning => vec![Capability::Planning, Capability::Architecture],
            TaskType::Debugging => vec![Capability::Debugging, Capability::CodeExplanation],
            TaskType::MultiFileEdit => vec![
                Capability::MultiFile,
                Capability::CodeGeneration,
                Capability::Agentic,
            ],
            TaskType::ShellTask => vec![Capability::ShellCommands, Capability::Agentic],
            TaskType::Explanation => vec![Capability::CodeExplanation],
        }
    }
    ...
    pub fn preferred_capabilities(&self) -> Vec<Capability> {
        match self {
            TaskType::CodeGeneration => vec![Capability::Agentic, Capability::MultiFile],
            TaskType::CodeReview => vec![Capability::Architecture],
            TaskType::Refactoring => vec![Capability::MultiFile, Capability::Agentic],
            TaskType::BugFix => vec![Capability::Agentic, Capability::ShellCommands],
            TaskType::TestWriting => vec![Capability::MultiFile],
            TaskType::Documentation => vec![Capability::CodeExplanation],
            TaskType::ArchitectureDesign => vec![Capability::MultiFile, Capability::Agentic],
            TaskType::Planning => vec![Capability::MultiFile],
            TaskType::Debugging => vec![Capability::ShellCommands, Capability::Agentic],
            TaskType::MultiFileEdit => vec![Capability::CodeSearch],
            TaskType::ShellTask => vec![Capability::Background],
            TaskType::Explanation => vec![Capability::Streaming],
        }
    }
            ...
            .map(|(i, p)| {
                let reason = if i == 0 {
                    format!(
                        ...
                        task_type,
                        p.match_score(&criteria.task_type.required_capabilities())
                            / criteria.task_type.required_capabilities().len() as u32
                    )
                } else {
...
mod tests {
    ...
    fn test_task_type_infer() {
        ...
        assert_eq!(
            TaskType::infer("refactor the auth module"),
            TaskType::Refactoring
        );
        assert_eq!(
            TaskType::infer("write tests for api"),
            TaskType::TestWriting
        );
        assert_eq!(
            TaskType::infer("add documentation"),
            TaskType::Documentation
        );
        assert_eq!(
            TaskType::infer("implement new feature"),
            TaskType::CodeGeneration
        );
    }
    ...
    fn test_selector_agentic() {
        let selector = ProviderSelector::new();
        let criteria = SelectionCriteria::for_task(TaskType::MultiFileEdit).agentic();
    ...
    fn test_selector_local() {
        let selector = ProviderSelector::new();
        let criteria = SelectionCriteria::for_task(TaskType::CodeGeneration).local();
    ...
    fn test_selector_exclude() {
        let selector = ProviderSelector::new();
        let criteria =
            SelectionCriteria::for_task(TaskType::CodeGeneration).exclude_provider("claude_code");
