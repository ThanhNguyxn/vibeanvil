//! Goose provider - Block/Square's AI coding agent
//!
//! Goose is an open-source autonomous AI agent from Block (Square)
//! that can execute complex coding tasks with MCP support.
//!
//! Installation:
//! - macOS: `brew install --cask block-goose`
//! - Other: https://github.com/block/goose
//!
//! Configuration:
//! - `GOOSE_MODEL`: Model to use (optional)
//! - `GOOSE_PROVIDER`: AI provider (openai, anthropic, etc.)
//! - `GOOSE_EXTRA_ARGS`: Additional CLI arguments

use super::{Context, Provider, ProviderResponse};
use anyhow::{anyhow, Result};
use async_trait::async_trait;
use std::process::Command;
use std::time::Duration;

/// Goose AI coding agent provider
pub struct GooseProvider {
    command: String,
    timeout: Duration,
}

impl Default for GooseProvider {
    fn default() -> Self {
        Self::new()
    }
}

impl GooseProvider {
    /// Create a new Goose provider
    pub fn new() -> Self {
        let timeout_secs = std::env::var("GOOSE_TIMEOUT_SECS")
            .ok()
            .and_then(|s| s.parse().ok())
            .unwrap_or(600);

        Self {
            command: std::env::var("GOOSE_COMMAND").unwrap_or_else(|_| "goose".to_string()),
            timeout: Duration::from_secs(timeout_secs),
        }
    }

    fn build_command(&self, prompt: &str, context: &Context) -> Command {
        let mut cmd = Command::new(&self.command);

        // Set working directory
        cmd.current_dir(&context.working_dir);

        // Use --message for single prompt execution (non-interactive)
        cmd.arg("--message").arg(prompt);

        // Add model if specified
        if let Ok(model) = std::env::var("GOOSE_MODEL") {
            cmd.arg("--model").arg(model);
        }

        // Add provider if specified
        if let Ok(provider) = std::env::var("GOOSE_PROVIDER") {
            cmd.arg("--provider").arg(provider);
        }

        // Add extra arguments
        if let Ok(extra_args) = std::env::var("GOOSE_EXTRA_ARGS") {
            for arg in extra_args.split_whitespace() {
                cmd.arg(arg);
            }
        }

        cmd
    }
}

#[async_trait]
impl Provider for GooseProvider {
    async fn execute(&self, prompt: &str, context: &Context) -> Result<ProviderResponse> {
        let mut cmd = self.build_command(prompt, context);

        // Execute with timeout
        let output = tokio::time::timeout(self.timeout, async {
            tokio::task::spawn_blocking(move || cmd.output())
                .await
                .map_err(|e| anyhow!("Task join error: {}", e))?
                .map_err(|e| anyhow!("Failed to execute goose: {}", e))
        })
        .await
        .map_err(|_| anyhow!("Goose timed out after {:?}", self.timeout))??;

        let stdout = String::from_utf8_lossy(&output.stdout).to_string();
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();

        let success = output.status.success();
        let errors = if stderr.is_empty() {
            vec![]
        } else {
            vec![stderr.clone()]
        };

        // Parse modified files from output
        let files_modified = parse_modified_files(&stdout);

        Ok(ProviderResponse {
            success,
            output: if success { stdout } else { stderr },
            errors,
            warnings: vec![],
            files_modified,
        })
    }

    async fn generate_commit_message(&self, diff: &str, context: &Context) -> Result<String> {
        let prompt = format!(
            "Generate a concise git commit message for this diff. \
             Return ONLY the commit message, no explanation:\n\n{}",
            diff
        );
        let response = self.execute(&prompt, context).await?;
        Ok(response.output.trim().to_string())
    }

    fn name(&self) -> &str {
        "goose"
    }

    fn is_available(&self) -> bool {
        which::which(&self.command).is_ok()
    }
}

/// Parse modified files from Goose output
fn parse_modified_files(output: &str) -> Vec<String> {
    let mut files = Vec::new();
    for line in output.lines() {
        // Look for patterns like "Created file: path" or "Modified: path"
        if let Some(path) = line.strip_prefix("Created file: ") {
            files.push(path.trim().to_string());
        } else if let Some(path) = line.strip_prefix("Modified: ") {
            files.push(path.trim().to_string());
        } else if let Some(path) = line.strip_prefix("Wrote ") {
            // Handle "Wrote path/to/file"
            if let Some(p) = path.split_whitespace().next() {
                files.push(p.to_string());
            }
        }
    }
    files
}

#[cfg(test)]
mod tests {
    use super::*;
    use std::path::PathBuf;

    fn cleanup_env() {
        std::env::remove_var("GOOSE_COMMAND");
        std::env::remove_var("GOOSE_MODEL");
        std::env::remove_var("GOOSE_PROVIDER");
        std::env::remove_var("GOOSE_EXTRA_ARGS");
        std::env::remove_var("GOOSE_TIMEOUT_SECS");
    }

    #[test]
    fn test_goose_provider_new() {
        cleanup_env();
        let provider = GooseProvider::new();
        assert_eq!(provider.command, "goose");
        assert_eq!(provider.timeout, Duration::from_secs(600));
    }

    #[test]
    fn test_goose_provider_name() {
        cleanup_env();
        let provider = GooseProvider::new();
        assert_eq!(provider.name(), "goose");
    }

    #[test]
    fn test_goose_build_command() {
        cleanup_env();
        let provider = GooseProvider::new();
        let context = Context {
            working_dir: PathBuf::from("/test"),
            session_id: "test-session".to_string(),
            contract_hash: None,
        };

        let cmd = provider.build_command("test prompt", &context);
        let args: Vec<_> = cmd.get_args().collect();

        assert!(args.contains(&std::ffi::OsStr::new("--message")));
        assert!(args.contains(&std::ffi::OsStr::new("test prompt")));
    }

    #[test]
    fn test_goose_with_model() {
        cleanup_env();
        std::env::set_var("GOOSE_MODEL", "claude-sonnet-4");
        let provider = GooseProvider::new();
        let context = Context {
            working_dir: PathBuf::from("/test"),
            session_id: "test-session".to_string(),
            contract_hash: None,
        };

        let cmd = provider.build_command("test", &context);
        let args: Vec<_> = cmd.get_args().collect();

        assert!(args.contains(&std::ffi::OsStr::new("--model")));
        assert!(args.contains(&std::ffi::OsStr::new("claude-sonnet-4")));
        cleanup_env();
    }

    #[test]
    fn test_parse_modified_files() {
        let output = r#"
Starting task...
Created file: src/main.rs
Modified: src/lib.rs
Wrote tests/test.rs successfully
Done.
"#;
        let files = parse_modified_files(output);
        assert_eq!(files.len(), 3);
        assert!(files.contains(&"src/main.rs".to_string()));
        assert!(files.contains(&"src/lib.rs".to_string()));
        assert!(files.contains(&"tests/test.rs".to_string()));
    }
}
